   ...
   if (l)
   {
	 ...
	 case EQUF:
            ...
            if (*label_margin == '\\')
            {
               ...
               if (x < 1)        return (int) 0;
               if (x > RADIX/32) return (int) 0;

               return (int) quadextractx(&l->l.value, x);
            }
            #endif

	    i = (int) quadextract(&l->l.value);
	 ...
	 case FORM:
	    ...
	    return (int) i;
         ...
         case LTAG:
            ...
            q = &locator[j];
            if (q->flags == 1) i += vextractq(q->runbank.p);
            return i;
            #endif
            #endif
...
int main(int argc, char *_argv[])
{
   ...
   fsize = lseek(ohandle, (int) 0, SEEK_CUR);
